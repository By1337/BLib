...
@NMSAccessor(forClazz = ParseCompoundTag.class, from = Version.V1_20_6, to = Version.V1_21_4)
public class ParseCompoundTagV1206 implements ParseCompoundTag {
    @Override
    ...
    public CompletableFuture<org.by1337.blib.nbt.impl.CompoundTag> readOfflinePlayerData(@NotNull UUID player) {
        return CompletableFuture.supplyAsync(() -> {
                    File file = new File(worldNBTStorage.getPlayerDir(), player.toString() + ".dat");
                    if (!file.exists() || !file.isFile()) return null;
                    try {
                        return MojangNbtReader.readCompressed(file);
                    } catch (IOException e) {
                        Bukkit.getLogger().log(Level.WARNING, "Failed to load player data for " + player, e);
                    }
                    return null;
                }
        );
    ...
    @Nullable
    public <T, R> R applyIfNotNull(@Nullable T raw, Function<T, R> function) {
        if (raw == null) return null;
        return function.apply(raw);
    }

    public Tag convert(NBT nbt) {
